...
- **WSADQE**: Перемещение камеры вперед, назад, влево, вправо, ввер, вниз.
...
- **Оптимизация производительности**
	- На данный момент внесены следующие улучшения для повышения эффективности и производительности:

- **Уменьшение количества итераций**: 
	- Ранее в рамках основного цикла игры (while (MainLoop)) существовало три вложенных цикла, каждый из которых проходил по всем клеткам игрового поля за кадр:
	- Загрузка данных на GPU для вычисления следующего поколения.
	- Обновление состояний клеток на CPU (дублирование работы GPU).
	- Обновление цветов клеток для отрисовки.

- **Теперь эти операции сведены к двум основным циклам**:
	- Загрузка данных на GPU и вычисление следующего поколения.
	- Непосредственное обновление цветов клеток после вычисления на GPU, что устраняет необходимость в третьем цикле.

- **Синхронизация обновления и отрисовки**:
	- Добавлен метод SetCellColor в классе GameOfLife, который позволяет устанавливать цвет клетки непосредственно после вычисления её нового состояния. 
