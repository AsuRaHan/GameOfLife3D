...
void main() {
    ...
    if (currentState == 1) { // Живая клетка
        if (neighbors >= 2 && neighbors <= 3) {
            nextState = 1; // Клетка выживает
        }
    } else { // Мертвая клетка
        if (neighbors == 3) {
            nextState = 1; // Клетка оживает
        }
    }
    ...
    if (nextState == 1) {
        colors[pos.y * gridWidth + pos.x] = vec3(0.0, 0.5, 0.0); // Зеленый для живых клеток
    } else {
        colors[pos.y * gridWidth + pos.x] = vec3(0.05, 0.05, 0.08); // Темный для мертвых клеток
    }
}
)";
...
void GPUAutomaton::SetupBuffers() {
    ...
    GL_CHECK(glGenBuffers(1, &colorsBuffer)); // Добавляем буфер для цветов
    // Проверка, что colorsBuffer успешно создан
    if (colorsBuffer == 0) {
    ...
    // Инициализация буфера для цветов
...
void GPUAutomaton::Update() {
    ...
    GL_CHECK(glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2, colorsBuffer)); // Привязка буфера цветов
